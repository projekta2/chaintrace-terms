<!--
══════════════════════════════════════════════════════════
  ChainTrace – Terms of Service & Support
  Author: Alexandre Iglesias · Projekta2 Creative Studio
══════════════════════════════════════════════════════════
-->

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:060c1a,25:0d1f4a,60:1e3a8a,100:2563eb&height=160&section=header&text=Terms%20of%20Service&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=ChainTrace%20%2B%20Support&descAlignY=55&descSize=18&descColor=ffffffcc" width="100%" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Status-Live-10b981?style=flat-square" />
  <img src="https://img.shields.io/badge/Version-2.1.0-2563eb?style=flat-square" />
  <img src="https://img.shields.io/badge/License-ONE--TIME_%2F_Lifetime-ff90e8?style=flat-square" />
  <img src="https://img.shields.io/badge/Last_Updated-2026--06--10-64748b?style=flat-square" />
</p>

<br />

## 1. Acceptance of terms

By installing, accessing, or using ChainTrace (the "Extension"), you agree to be bound by these Terms of Service. If you do not agree, do not install or use the Extension.

<br />

## 2. License models

### Free plan
- 50 captures per month (resets on the 1st of each month)
- 500 rows max per capture
- Up to 3 saved Recipes
- Valid forever – no expiration

### Premium plan
- Unlimited captures and rows
- Unlimited Recipes
- Regex validation and transform rules
- REST API export with custom headers
- Priority support (email)
- **One‑time payment** – lifetime access, no subscription, no renewal

Premium licenses are sold via [Gumroad](https://gumroad.com/l/chaintrace-premium). Licenses are verified through Gumroad's API. You may use one license key per user. Volume discounts are available for teams – contact **support@chaintrace.app**.

<br />

## 3. Refund policy

**30‑day money‑back guarantee** – If ChainTrace does not meet your expectations, or if it fails to save you at least 5 hours per month, you are eligible for a full refund within 30 days of purchase.

To request a refund, email **support@chaintrace.app** with your Gumroad receipt and a short explanation. No lengthy forms, no interrogations – we refund every cent.

<br />

## 4. Acceptable use

You may use ChainTrace to extract data from any website you are legally authorised to access. You may not:

- Use the Extension to bypass paywalls, authentication systems, or any access restrictions.
- Use the Extension to scrape data at a rate that harms the source website.
- Reverse engineer, decompile, or attempt to extract the source code of the Extension (except for personal inspection).
- Resell or redistribute the Extension or Premium licenses without explicit written permission.

<br />

## 5. Disclaimer of warranties

The Extension is provided "as is" and "as available", without warranty of any kind, express or implied. We do not guarantee that the Extension will work on every website, that data extraction will be error‑free, or that pagination detection will succeed on every portal.

<br />

## 6. Limitation of liability

To the maximum extent permitted by law, ChainTrace and its creator shall not be liable for any indirect, incidental, special, consequential, or punitive damages, or any loss of profits or revenues, whether incurred directly or indirectly, or any loss of data, use, goodwill, or other intangible losses, resulting from (i) your use or inability to use the Extension; (ii) any errors or inaccuracies in extracted data; (iii) unauthorised access to or alteration of your data; or (iv) any other matter relating to the Extension.

<br />

## 7. Support

Support is provided via email:

- **Free plan**: Community‑based support (response within 5 business days)
- **Premium plan**: Priority support (response within 24‑48 hours, Monday–Friday)

For all support inquiries, contact **support@chaintrace.app**. Please include:

- Your ChainTrace version (shown in the extension popup)
- The URL of the supplier portal you are trying to capture
- A brief description of the issue (and, if possible, a screenshot)

We read every message and respond personally. No bots, no automated ticketing systems.

<br />

## 8. Changes to these terms

We may update these Terms from time to time. The latest version will always be available at [chaintrace.app/terms](https://chaintrace.app/terms). Continued use of the Extension after changes constitutes acceptance of the new Terms.

<br />

## 9. Governing law

These Terms shall be governed by and construed in accordance with the laws of Spain, without regard to its conflict of law provisions.

<br />

## 10. Contact

For legal questions, abuse reports, or any matter not covered by support:

📧 **legal@chaintrace.app**  
🌐 [chaintrace.app](https://chaintrace.app)

<br />

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:2563eb,100:060c1a&height=80&section=footer" width="100%" />
</p>

<p align="center">
  <sub>ChainTrace – Built with care for supply chain professionals. Girona, Spain.</sub>
</p>
